...
For now code is not compiled to ES5 and if used on browser you might need to compile it by yourself
    ...
    methods: {
      startConversation: singular(function(done) {
        ...
        ChatService.conversation.createDirect(this.professor.id)
          ...
          .then(done); // In this case done is called asynchronously after api call have finished with
      })
    }
  };
```
